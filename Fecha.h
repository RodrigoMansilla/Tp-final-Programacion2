...
int Fecha::validar_fecha(){
    int valido=1;
    int vmes[]={31,28,31,30,31,30,31,31,30,31,31,31};
    if(esbisiesto(anio)) vmes[1]=29;
    if(dia<1 || dia>vmes[mes-1]) valido=0;
    if(mes<1 || mes>12) valido =0;
    if(anio<1901) valido=0;
    return valido;
}
...
{
    return anio > 0 && !(anio % 4) && (anio % 100 || !(anio % 400));
};
int calcular_dias(int mes, int anio)
{
    static const int dias_mes[2][13] = {
    ...
    return dias_mes[biciesto(anio)][mes - 1];
}
